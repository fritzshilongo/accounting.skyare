...
<section class="hero-card">
    <h1 class="hero-title">Create Estimate</h1>
    <p class="hero-copy">Prepare a proposal with client, product, and commercial terms.</p>
</section>

<section class="form-card">
    <form id="estimate-form" method="POST" action="/estimates" class="form-grid two">
        @csrf
        <div>
        ...
        <div>
            ...
            <input id="issue_date" type="date" name="issue_date" value="{{ old('issue_date', now()->toDateString()) }}" required>
        </div>
        <div>
    ...
    <section class="table-card" style="margin-top:24px;">
        ...
        <p class="hero-copy" style="margin-top:8px;">Create a multi-item estimate with quantity, price, and description per row.</p>
            ...
            <table class="table" id="estimate-items-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <select name="item_product_id[]" class="item-product-select" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->product_id }}" data-price="{{ number_format($product->price, 2, '.', '') }}" data-description="{{ htmlspecialchars($product->description ?? '', ENT_QUOTES) }}">{{ $product->name }} · {{ number_format($product->price, 2) }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="number" step="0.01" min="0.01" name="item_quantity[]" class="item-quantity" value="{{ old('item_quantity.0', 1) }}" required>
                        </td>
                        <td>
                            <input type="number" step="0.01" min="0" name="item_price[]" class="item-price" value="{{ old('item_price.0', 0) }}" required>
                        </td>
                        <td class="line-total-cell">{{ $_SESSION['user']['currency_symbol'] ?? 'N$' }}0.00</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="line-total-cell" id="estimate-grand-total">{{ $_SESSION['user']['currency_symbol'] ?? 'N$' }}0.00</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
    ...
    <div class="toolbar-left span-full">
        ...
        <a href="/estimates" class="btn btn-secondary">Cancel</a>
    </div>
    </form>
</section>
